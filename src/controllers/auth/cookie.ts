...
export const logoutUserWithCookie = asyncErrorHandler(async (req: UserRequest, res: Response) => {
    ...
    if (!user) {
        ...
        throw new APIError("Brukeren er logget ut", HTTPStatusCode.OK);
    };
    ...
    res.clearCookie(refreshTokenCookieName, { httpOnly: true });
    return res.status(200).json({ message: "Brukeren er logget ut" });
});
...
export const refreshToken = asyncErrorHandler(async (req: Request, res: Response) => {
    ...
    if (!refreshToken) throw new APIError("Uautorisert", HTTPStatusCode.UNAUTHORIZED);
    ...
    if (!user) throw new APIError("Uautorisert", HTTPStatusCode.UNAUTHORIZED);

    jwt.verify(
        refreshToken as string,
        refreshTokenSecret || '',
        (error: VerifyErrors | null, decoded: DecodedUser | any) => {
            if (error || user.username !== decoded.username) throw new APIError("Uautorisert", HTTPStatusCode.UNAUTHORIZED);
